...
public class SharedPerefereUtility {

    private static final String PREF_FILE = "com.example.rumahmakan.mode.DATA";
    ...
    */
     ...
     */
    public static List<Makanan> getAllLis(Context ctx) {
        String jsonString = getSharedPref(ctx).getString(TRANS_KEY, null);
        List<Makanan> trs = new ArrayList<Makanan>();
        if (jsonString != null) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    trs.add(Makanan.fromJSONObject(obj));
                }
    ...
    /*
        ...
        Perhatikan bahwa data disimpan dalam format JSON String
     */
    private static void saveAllTransaksi(Context ctx, List<Makanan> trs) {
     ...
     */
    public static void addLis(Context ctx, Makanan tr) {
        List<Makanan> trs = getAllLis(ctx);
        trs.add(tr);
